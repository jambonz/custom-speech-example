...
const { StreamingRecognitionRequest, RecognitionConfig,
  RecognitionSpec, AudioEncoding } = require('../../stubs/vosk/vosk_stt_pb');
    ...
    try {
      if (!isBinary) {
        ...
        if (obj.type === 'start') {
          ...
          spec.setAudioEncoding(AudioEncoding.LINEAR16_PCM);
        ...
        if (socket.voskConnection) {
          const { voskClient, config, jambonz_req } = socket.voskConnection;
          socket.audioBuffer.push(data);
          if (socket.audioBuffer.length > 4) {
            const data = Buffer.concat(socket.audioBuffer);
            socket.audioBuffer = [];
            const request = new StreamingRecognitionRequest();
            request.setConfig(config);
            request.setAudioContent(data);

            voskClient.streamingRecognize(request, (err, response) => {
              if (err) {
                console.error(err);
              }
